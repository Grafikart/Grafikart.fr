...
class Course extends Model implements RegisterMedia, Revisionable, Searchable
{
    ...
    {
        return intval(ceil(self::query()->published(true)->selectRaw('SUM(duration) as duration')->value('duration') / 3600));
    }
    ...
    {
        if ($forceHTML || $this->isPremium() || $this->isScheduled()) {
            return $this->videoUrl();
        }

        return $this->youtube_id ?? $this->videoUrl();
    }
}

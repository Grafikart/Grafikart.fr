...
export function Search() {
    ...
    return (
        <CommandDialog
            ...
            className="max-w-100 top-10 w-full translate-y-0"
        >
            <SearchCommand />
        </CommandDialog>
    );
...
export function SearchCommand(props: {
    ...
    const { data, isFetching } = useApiFetch<SearchResultData[]>(
        ...
        {
            enabled:
                debouncedSearch.length > 1 && !debouncedSearch.endsWith(':'),
            staleTime: 5_000,
        },
    );
    const items = data ?? [];
    return (
        ...
        >
            ...
            <CommandList>
                {isFetching && (
                    <div className="text-muted-foreground absolute right-3 top-1 flex justify-center py-2">
                        <Spinner />
                    </div>
                )}
                {data && (

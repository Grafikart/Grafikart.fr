...
export const NodesInput = (props: Props) => {
    ...
    const onAdd = (item: {name: string, type: string, icon?: string, id: number|null}) => {
        if (!addPosition) {
            return;
        }
        const node = addNode({
                ...
                <GraphContext value={useMemo(() => ({ updateNode }), [updateNode])}>
                    ...
                    {edge && (
                        <FlowEdgeMenu
                            edge={edge}
                            position={position}
                            onEdgeChange={(e) => { updateEdge(e); reset(); }}
                        />
                    )}
                </GraphContext>
            </div>
            <textarea readOnly name="nodes" className="w-full min-h-100 border p-2" value={JSON.stringify(value, null, 2)} />
        </div>
    );
    }

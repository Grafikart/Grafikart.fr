...
type Props = {
    onSelect: (v: {name: string, type: string, icon?: string, id: number|null}) => void,
    onClose: () => void,
}
...
export function FlowAdd (props: Props) {
    ...
    return <div className="absolute inset-0 bg-background/40 z-10 grid justify-center" onClick={onOverlayClick}>
        <div className="flex gap-2">
        <SearchCommand onSelect={props.onSelect} />
            <Button onClick={() => addCustomType('fork')} variant="outline" size="icon" className="mt-1">
                <GitForkIcon />
            </Button>
            <Button variant="outline" size="icon" className="mt-1" onClick={() => addCustomType('gate')}>
                <ConstructionIcon />
            </Button>
        </div>
    </div>

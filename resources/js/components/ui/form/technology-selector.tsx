...
export function TechnologySelector(props: Props) {
  ...
  const { data, isFetching } = useApiFetch<TechnologyUsageData[]>(
    TechnologyController.index({ query: { q: debouncedSearch } }).url,
    {
  ...
  return (
    <fieldset className="space-y-3" id={props.id}>
      <div
        ...
        style={{ gridTemplateColumns: "1fr 80px 2rem max-content" }}
      >
        {items.map((item, k) => (
...
type ItemProps = {
  item: TechnologyUsageData
  k: number
  onToggle: (item: TechnologyUsageData) => void
}

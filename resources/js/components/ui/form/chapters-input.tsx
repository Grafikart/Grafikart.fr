...
export function ChaptersInput({ defaultValue, name = "chapters" }: Props) {
    ...
    setChapters((prev) =>
      prev.map((chapter, i) =>
        i === chapterIndex
          ? {
              ...chapter,
              courses: chapter.courses.filter((_, ci) => ci !== courseIndex),
            }
          : chapter,
      ),
    )
  ...
  const setCourses = (chapterIndex: number, courses: OptionItemData[]) => {
    setChapters((prev) =>
      prev.map((chapter, i) =>
        i === chapterIndex ? { ...chapter, courses: courses } : chapter,
      ),
    )
  }
    ...
    <div className="flex flex-wrap items-start gap-4">
      {chapters.map((chapter, chapterIndex) => (
        <ChapterItem
          ...
          onReorderCourse={(courses) => setCourses(chapterIndex, courses)}
        />
      ))}
      <input
  ...
  const handleAdd: KeyboardEventHandler<HTMLInputElement> = async (e) => {
    if (e.key === "Enter") {
      ...
      try {
        ...
        const item = await apiFetch<OptionItemData>(
          CourseController.show(input.valueAsNumber).url,
        )
        input.value = ""
        onAddCourse(item)
      } catch (e) {
  ...
  return (
    <div className="grid grid-cols-1 gap-2 w-70">
      ...
      <div className="grid grid-cols-1 gap-2">
        <Sortable
          value={chapter.courses}
          getItemValue={(c) => c.id.toString()}
          onValueChange={onReorderCourse}

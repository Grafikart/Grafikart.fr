...
export function NotificationsMenu(props: Props) {
    ...
    return (
        <Menu.Root open={open} onOpenChange={setOpen}>
            {unread > 0 && (
                <div className="bg-destructive absolute -right-1 -top-1 grid size-4 place-items-center rounded-full text-xs text-white">
                    {unread}
                </div>
            )}
            <Menu.Trigger
            ...
            <Menu.Portal className="z-1000">
                <Menu.Positioner sideOffset={8}>
                    <Menu.Popup className="w-85 bg-card z-1000 data-starting-style:scale-90 data-starting-style:opacity-0 data-ending-style:scale-90 data-ending-style:opacity-0 rounded-sm border text-sm outline-none transition-all">
                        <Menu.Arrow>
                            <ArrowSvg />
                        </Menu.Arrow>
                        <div className="text-muted border-b py-2 text-center">
                            Nouveaux messages
                        </div>
                        ...
                        <Menu.Item
                            className="hover:bg-background block border-t p-2 text-center"
...
function NotificationItem({
    ...
    return (
        <Menu.Item
            ...
            className={cn(
                'hover:bg-background [&_strong]:text-foreground-title focus-visible:ring-primary relative block border-b p-4 pr-7 last:border-b-0 focus-visible:ring',
                !read && 'bg-primary/5',
            )}
...
function ArrowSvg(props: ComponentProps<'svg'>) {
    return (
        <svg
            ...
            viewBox="0 0 20 10"
            fill="none"
            className="text-card absolute -top-2.5 left-1/2 -translate-x-2.5"
            {...props}
        >
            <path

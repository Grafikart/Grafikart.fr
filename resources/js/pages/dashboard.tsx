        ...
        <TableBody>
          {jobs.map((job) => (
            <TableRow key={job.id}>
              ...
              {type === "failed" && (
                <TableCell>
                  <div className="line-clamp-3 max-w-50 overflow-hidden whitespace-pre-wrap text-destructive">
                    {job.exception}
                  </div>
                </TableCell>
              )}
              ...
              <TableCell className="flex justify-end gap-1">
                ...
                <ButtonLink
                  ...
                  size="icon"
                  href={
                    type === "failed" ? destroyFailed(job.id) : destroy(job.id)
                  }
        ...
        <CardContent className="py-0">
          ...
          <TabsContent value="year">
            <SimpleChart
              data={months}
              formatter={(v) => {
                return new Date(v.year, v.month - 1, 10).toLocaleDateString(
                  "fr-FR",
                  {
                    month: "long",
                  },

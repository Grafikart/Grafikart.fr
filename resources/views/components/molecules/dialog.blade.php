...
<dialog
    ...
    <div
        class="{{ cn(['bg-card relative z-3 rounded-xl max-w-[calc(100%-2rem)] max-h-[calc(100%-2rem)] w-340 h-fit p-4 opacity-0 translate-y-4 group-open:opacity-100 group-open:translate-y-0 starting:translate-y-4 starting:opacity-0 duration-300', $class]) }}"
        id="{{ $id }}">
        @if($title)

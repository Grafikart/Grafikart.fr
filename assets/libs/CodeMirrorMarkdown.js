...
CodeMirror.defineMode(
  'markdown',
  (cmCfg, modeCfg) => {
    let htmlMode = CodeMirror.getMode(cmCfg, 'text/html')
    let htmlModeMissing = htmlMode.name == 'null'

    function getMode (name) {
      if (CodeMirror.findModeByName) {
        let found = CodeMirror.findModeByName(name)
        if (found) name = found.mime || found.mimes[0]
      }
      let mode = CodeMirror.getMode(cmCfg, name)
    ...
    function blankLine (state) {
      ...
      if (state.f == htmlBlock) {
        ...
        if (!exit) {
          let inner = CodeMirror.innerMode(htmlMode, state.htmlState)
          exit =
            inner.mode.name == 'xml' &&
    ...
    function inlineNormal (stream, state) {
      ...
      if (modeCfg.xml && ch === '<' && stream.match(/^\/\w*?>/)) {
        ...
        let len = 1
        let before = stream.pos == 1 ? ' ' : stream.string.charAt(stream.pos - 2)
        while (len < 3 && stream.eat(ch)) len++
        let after = stream.peek() || ' '
        // See http://spec.commonmark.org/0.27/#emphasis-and-strong-emphasis
        let leftFlanking =
          !/\s/.test(after) && (!punctuation.test(after) || /\s/.test(before) || punctuation.test(before))
        let rightFlanking =
          !/\s/.test(before) && (!punctuation.test(before) || /\s/.test(after) || punctuation.test(after))
        let setEm = null
        let setStrong = null
        if (len % 2) {
          // Em
